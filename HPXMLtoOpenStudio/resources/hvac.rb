...
module HVAC
  ...
  def self.apply_heating_system(runner, model, weather, spaces, hpxml_bldg, hpxml_header, schedules_file, airloop_map,
    ...
    get_hpxml_hvac_systems(hpxml_bldg).each do |hvac_system|
      ...
      when HPXML::HVACTypeBoiler
        airloop_map[sys_id] = apply_boiler(runner, model, hpxml_bldg, heating_system, hvac_sequential_load_fracs, conditioned_zone, hvac_unavailable_periods)
      when HPXML::HVACTypeElectricResistance
        apply_electric_baseboard(model, hpxml_bldg, heating_system, hvac_sequential_load_fracs, conditioned_zone, hvac_unavailable_periods)
      when HPXML::HVACTypeStove, HPXML::HVACTypeSpaceHeater, HPXML::HVACTypeWallFurnace,
        ...
        apply_unit_heater(model, hpxml_bldg, heating_system, hvac_sequential_load_fracs, conditioned_zone, hvac_unavailable_periods)
      end
  ...
  def self.apply_heat_pump(runner, model, weather, spaces, hpxml_bldg, hpxml_header, schedules_file, airloop_map,
    ...
    get_hpxml_hvac_systems(hpxml_bldg).each do |hvac_system|
      ...
      when HPXML::HVACTypeHeatPumpWaterLoopToAir
        airloop_map[sys_id] = apply_water_loop_to_air_heat_pump(model, hpxml_bldg, heat_pump, hvac_sequential_load_fracs, conditioned_zone, hvac_unavailable_periods)
  ...
  # @param hvac_unavailable_periods [Hash] Map of htg/clg => HPXML::UnavailablePeriods for heating/cooling
  # @return [OpenStudio::Model::AirLoopHVAC] The newly created air loop hvac object
  def self.apply_water_loop_to_air_heat_pump(model, hpxml_bldg, heat_pump, hvac_sequential_load_fracs, control_zone, hvac_unavailable_periods)
  ...
  def self.apply_boiler(runner, model, hpxml_bldg, heating_system, hvac_sequential_load_fracs, control_zone, hvac_unavailable_periods)
  ...
  # @param hvac_unavailable_periods [Hash] Map of htg/clg => HPXML::UnavailablePeriods for heating/cooling
  # @return [nil]
  def self.apply_electric_baseboard(model, hpxml_bldg, heating_system, hvac_sequential_load_fracs, control_zone, hvac_unavailable_periods)
  ...
  # @param hvac_unavailable_periods [Hash] Map of htg/clg => HPXML::UnavailablePeriods for heating/cooling
  # @return [nil]
  def self.apply_unit_heater(model, hpxml_bldg, heating_system, hvac_sequential_load_fracs, control_zone, hvac_unavailable_periods)
  ...
  def self.add_fan_power_ems_program(model, fan, hp_min_temp)
    # Sensors
    tout_db_sensor = model.getEnergyManagementSystemSensors.find { |s| s.additionalProperties.getFeatureAsString('ObjectType').to_s == Constants::ObjectTypeSensorSiteOutdoorAirDBTemp }
  ...
  def self.add_supplemental_coil_ems_program(model, htg_supp_coil, htg_coil, has_deadband_control, cooling_system)
    ...
    htg_sp_ss = model.getEnergyManagementSystemSensors.find { |s| s.additionalProperties.getFeatureAsString('ObjectType').to_s == Constants::ObjectTypeSensorIndoorHeatingSetpointTemp }
  ...
  def self.add_two_speed_staging_ems_program(model, unitary_system, htg_supp_coil, has_deadband_control, cooling_system)
    ...
    # Sensors
    living_temp_ss = model.getEnergyManagementSystemSensors.find { |s| s.additionalProperties.getFeatureAsString('ObjectType').to_s == Constants::ObjectTypeSensorIndoorAirDBTemp }

    htg_sp_ss = model.getEnergyManagementSystemSensors.find { |s| s.additionalProperties.getFeatureAsString('ObjectType').to_s == Constants::ObjectTypeSensorIndoorHeatingSetpointTemp }
    clg_sp_ss = model.getEnergyManagementSystemSensors.find { |s| s.additionalProperties.getFeatureAsString('ObjectType').to_s == Constants::ObjectTypeSensorIndoorCoolingSetpointTemp }
  ...
  def self.add_variable_speed_power_ems_program(runner, model, air_loop_unitary, heating_system, cooling_system, htg_supp_coil, clg_coil, htg_coil, schedules_file)
    ...
    indoor_temp_sensor = model.getEnergyManagementSystemSensors.find { |s| s.additionalProperties.getFeatureAsString('ObjectType').to_s == Constants::ObjectTypeSensorIndoorAirDBTemp }
    ...
    [htg_coil, clg_coil].each do |coil|
      ...
      if coil.is_a? OpenStudio::Model::CoilHeatingDXMultiSpeed
        ...
        outdoor_db_sensor = model.getEnergyManagementSystemSensors.find { |s| s.additionalProperties.getFeatureAsString('ObjectType').to_s == Constants::ObjectTypeSensorSiteOutdoorAirDBTemp }
        outdoor_w_sensor = model.getEnergyManagementSystemSensors.find { |s| s.additionalProperties.getFeatureAsString('ObjectType').to_s == Constants::ObjectTypeSensorSiteOutdoorAirRH }
        outdoor_bp_sensor = model.getEnergyManagementSystemSensors.find { |s| s.additionalProperties.getFeatureAsString('ObjectType').to_s == Constants::ObjectTypeSensorSiteOutdoorAirBarPressure }
  ...
  def self.add_installation_quality_ems_program(model, heating_system, cooling_system, air_loop_unitary, htg_coil, clg_coil)
    ...
    tin_sensor = model.getEnergyManagementSystemSensors.find { |s| s.additionalProperties.getFeatureAsString('ObjectType').to_s == Constants::ObjectTypeSensorIndoorAirDBTemp }
    tout_sensor = model.getEnergyManagementSystemSensors.find { |s| s.additionalProperties.getFeatureAsString('ObjectType').to_s == Constants::ObjectTypeSensorSiteOutdoorAirDBTemp }
